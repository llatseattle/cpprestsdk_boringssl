...
#include "grpc/route_guide/cpp/helper.h"
#include "grpc/route_guide/proto/route_guide.grpc.pb.h"
...
int main(int argc, char **argv) {
  ...
  while (restart_count_ < 10) {
    ++restart_count_;
    std::cout << "------ GetFeature for the " << restart_count_
              << " time ------" << std::endl;
    guide.GetFeature();
    std::this_thread::sleep_for(std::chrono::seconds(1));
    std::cout << "GetFeature Done.\n";
  }

  restart_count_ = 0;
  while (restart_count_ < 10) {
    ++restart_count_;
    std::cout << "------ ListFeatures for the " << restart_count_
              << " time ------" << std::endl;
    guide.ListFeatures();
    std::this_thread::sleep_for(std::chrono::seconds(1));
    std::cout << "ListFeatures Done.\n";
  }

  restart_count_ = 0;
  while (restart_count_ < 10) {
    ++restart_count_;
    std::cout << "------ RouteChat for the " << restart_count_ << " time ------"
              << std::endl;
    guide.RouteChat();
    std::this_thread::sleep_for(std::chrono::seconds(1));
    std::cout << "RouteChat Done.\n";
  }
